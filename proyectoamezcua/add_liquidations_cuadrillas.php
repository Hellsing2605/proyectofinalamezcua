...
$cuadrilla_inventory = [];
$cuadrilla_id = null;
if (isset($_POST['cuadrilla_id']) && (int)$_POST['cuadrilla_id'] > 0) {
    $cuadrilla_id = (int)$_POST['cuadrilla_id'];
    $inventory = find_cuadrilla_inventory($cuadrilla_id);
    foreach ($inventory as $item) {
        if ($item['quantity'] > 0) {
            $cuadrilla_inventory[] = $item;
        }
    }
}
...
if (isset($_POST['add_liquidation'])) {
    ...
    if (empty($errors)) {
        $l_code = generate_liquidation_code();
        ...
        if (!$all_products_valid) {
            ...
            redirect('add_liquidations_cuadrillas.php', false);
        }

        $query  = "INSERT INTO liquidations (liquidation_code, supervisor_id, date, warehouse_id, technician_id, cuadrilla_id, obra_id, observations) VALUES (";
        $query .= " '{$l_code}', '{$supervisor_id}', '{$l_date}', '{$warehouse_id}', ";
        $query .= isset($technician_id) ? "'{$technician_id}', " : "NULL, ";
        $query .= isset($cuadrilla_id) ? "'{$cuadrilla_id}', " : "NULL, ";
        $query .= isset($obra_id) ? "'{$obra_id}', " : "NULL, ";
        $query .= "'{$observations}')";
        ...
        if ($db->query($query)) {
          ...
          foreach ($_POST['product'] as $index => $product_id) {
            ...
            if ($product_id && $quantity) {
                // Obtener las transferencias actuales de la cuadrilla en el almacén
                $current_transfers = find_cuadrilla_inventory_by_warehouse($cuadrillas_id, $warehouse_id, $product_id);
                ...
                $product_liquidaciones = find_product_liquidaciones_c($cuadrillas_id, $warehouse_id, $product_id);
    ...
    <div class="col-md-8">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form method="post" action="add_liquidations_cuadrillas.php">
                    <div class="form-group">
                        <label for="liquidation-code">Código de Liquidación</label>
                        <input type="text" class="form-control" name="liquidation-code" value="<?php echo generate_liquidation_code(); ?>" readonly>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label for="cuadrilla_id">Se realiza la liquidación por parte de</label>
